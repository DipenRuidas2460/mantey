...
import { postData, stockInfo, fetchData } from "~/lib/clientFunctions";
...
const Product = ({
  ...
  const imgs = [product?.image[0]?.url, product?.gallery[0]?.url];
  const [bcsImage, setBcsImage] = useState(imgs[0]);
        ...
        <div>
          <Link href={`/product/${product.slug}`}>
            <div
              ...
              onMouseEnter={() => setBcsImage(imgs[1])}
              onMouseLeave={() => setBcsImage(imgs[0])}
            >
              <ImageLoader
                src={bcsImage}
                alt={product.name}
          ...
          {product.discount < product.price && (
            <div className={c.discount}> {discountInPercent}% OFF</div>
          )}
          <div className={c.nameContainer}>
            ...
            >
              ...
              <div className={c.type}>{product.type}</div>
