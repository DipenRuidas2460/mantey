...
const Sidebar = (props) => {
  ...
  return (
    <li className={classes.list} style={{border:"10px solid red"}}>
      <button
        className={`${props.active ? classes.parent_button_active : classes.parent_button
          }`}
            ...
            ? props.sub.map((subCategory, index) => (
              <li key={subCategory.slug + index} className={classes.sublist}>
                <button
                  ...
                  className={`${clicked === index ? classes.subnav_active : classes.subnav
                    }`}
                >
                  {subCategory.name}
                </button>
