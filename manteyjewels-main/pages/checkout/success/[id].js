...
const OrderSuccessPage = ({ orId }) => {
  ...
  if (router.query.id != undefined) {
    url = `/api/home/order?id=${router.query.id}`;
    mainData = useSWR(router.query.id ? url : null, fetchData);
  } else {
    url = `/api/home/fetch-order?id=${orId}`;
    mainData = useSWR(orId ? url : null, fetchData);
  }

  const { data, error } = mainData;
