...
const EditTag = (props) => {
  ...
  const url = `/api/tags/edit?id=${router.query.id}`;
  ...
  useEffect(() => {
    if (data && data.tag) {
      const attr_value = data.tag.values
        ? data.tag.values
        : [{ name: "", value: "" }];
      setAttrData(data.tag);
      setInputList(attr_value);
      setIsChecked(data.tag?.hideAttribute)
    }
  }, [data]);
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      formData.append("hideAttribute", isChecked)
      const response = await postData("/api/tags/edit", formData);
      response.success
  ...
  return (
    <>
      {error ? (
        ...
        <div>
          <h4 className="text-center pt-3 pb-5">{t("Edit Tag")}</h4>
          <form onSubmit={submitHandler}>
            ...
            <div className="card">
              <div className="card-header">{t("Tag Value")}</div>
...
EditTag.requireAuthAdmin = true;
EditTag.dashboard = true;
