...
const NewTag = () => {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      const response = await postData("/api/tags", formData);
      response.success
  ...
  return (
    <>
      <h4 className="text-center pt-3 pb-5">{t("Create New Tags")}</h4>
      <form id="attr_form" onSubmit={submitHandler}>
        ...
        <div className="card">
          <div className="card-header">{t("Tags Value")}</div>
...
NewTag.requireAuthAdmin = true;
NewTag.dashboard = true;
