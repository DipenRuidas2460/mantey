...
const Tags = () => {
  ...
  useEffect(() => {
    setPermissions(cpf(session, "tag"));
  }, [session]);
  ...
  const deleteProduct = async () => {
    setIsOpen(false);
    await deleteData(`/api/tags?id=${selectedAttr}`)
      .then((data) =>
        data.success
    ...
    },
    {
      ...
      selector: (row) => (
        <div>
          ...
          {permissions.edit && (
            <Link href={`/dashboard/tags/${row._id}`}>
  ...
  return (
    <>
      {error ? (
        ...
        <div>
          <h4 className="text-center pt-3 pb-5">All Tags</h4>
