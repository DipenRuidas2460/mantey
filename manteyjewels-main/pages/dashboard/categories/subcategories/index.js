...
const SubcategoryTable = (props) => {
    ...
    if (data && data.category) {
      ...
      data.category.map((category) =>
        category.subCategories.map((sub) => {
          ...
          sub.original = category.subCategory?.original
          subList.push(sub);
        })
      );
  ...
  const columns = [
    {
      name: t("name"),
      selector: (row) => row?.subCategory?.original,
      sortable: true,
    },
    {
      name: t("slug"),
        ...
        <div>
          {permissions.delete && (
            <div
              ...
              onClick={() => openModal(row.catId, row.slug, row?.subCategory?.original, "delete")}
            >
          ...
          {permissions.edit && (
            <div
              className={classes.button}
              onClick={() => openModal(row.catId, row.slug, row?.subCategory?.original, "edit")}
            >
